...
class Data:
    ...
    def save(self, path='', final_save=True):
        ...
        # EREZ ADDED FOR COMPATIBILITY 
        #print("path: ")
        #print(path)
        extra_path=''
        if (path != '') and not(path.startswith('temp')):
            extra_path = path
            path =''
        ...
        if final_save:
            ...
            os.makedirs(os.path.dirname(final_file_name), exist_ok=True)  # ADDED
        ...
        # EREZ ADDED 
        if (extra_path != '') and not(extra_path.startswith('temp')):
            dest = os.path.join(extra_path, self.get_file_name() + '.json.zip')
            os.makedirs(os.path.dirname(dest), exist_ok=True)  # ADDED

            copyfile(final_file_name + '.zip', dest)
    ...
    def temp_save(self):
        ...
        path ='temp/' + str(self.temp_saves)
        print("path:======================== ")
        print(path)
        self.save(path=path, final_save=False)
