...
class CriticNet:
    ...
    # EREZ ADDED 
    def save(self, path):    # add args so we can havea bunch of these for different modles
        print('\tSaving Critic Net')  # look at code from action branching!

        # this will save the sesssion -- but how do we get to everything else ?? 
        file = os.path.join(path, "critic_net.ckpt")
        with self.g.as_default():
            saver = tf.train.Saver()
            saved_path = saver.save(self.sess, file)
            print('Critic Net model saved in {}'.format(saved_path))

    # EREZ ADDED 
    def restore(self, file):    # or dir and make it find it 
        #self.g = tf.Graph()
        #self.sess = tf.InteractiveSession()
        with self.g.as_default():
            saver = tf.train.Saver()
